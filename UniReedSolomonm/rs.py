...
try: # Cython implementation import. This should be a bit faster than using PyPy with the pure-python implementation.
    ...
    from UniReedSolomonm.cpolynomial import Polynomial
...
class RSCoder(object):
    def __init__(self, n, k, generator=2, prim=0x1100b, fcr=1, c_exp=16):
    ...
    def encode(self, message, poly=False, k=None, return_string=True):
        ...
        if not poly:
            ...
            if return_string and self.gf2_charac < 65536:
    ...
    def decode(self, r, nostrip=False, k=None, erasures_pos=None, only_erasures=False, return_string=True):
        ...
        if return_string and self.gf2_charac < 65536: # automatically disable return_string if the field is above 255 (chr would fail, so it's up to the user to define the mapping)
