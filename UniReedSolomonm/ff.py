...
GF2_charac = int(2**16 - 1)
GF2_c_exp = 16

# Exponent table for generator=3 and prim=0x11b in GF(2^8)
GF2int_exptable = []
with open('UniReedSolomonm/gf216exp.txt') as f:
    for line in f:
        for s in line.split(', '):
            GF2int_exptable.append(int(s))

GF2int_exptable = array.array("i", GF2int_exptable)


logtable = []
logtable.append(-1)
with open('UniReedSolomonm/gf216log.txt') as g:
    for line in g:
        for r in line.split(', '):
...
def init_lut(generator=2, prim=0x1100b, c_exp=16):
...
class GF2int(int):
    ...
    def multiply(a, b, prim=0x1100b, field_charac_full=65536, carryless=True):
    ...
    def multiply_slow(x, y, prim=0x1100b):
